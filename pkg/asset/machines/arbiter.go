...
const (
	...
	arbiterSecretFileName              = "99_openshift-cluster-api_arbiter_host-bmc-secrets-%s.yaml"
	arbiterNetworkConfigSecretFileName = "99_openshift-cluster-api_arbiter_host-network-config-secrets-%s.yaml"
)
...
func (m *Arbiter) Generate(ctx context.Context, dependencies asset.Parents) error {
	...
	arbiterUserDataSecretName := "arbiter-user-data"
	...
	case awstypes.Name:
		...
		mpool := defaultAWSMachinePoolPlatform("arbiter")
		...
		if mpool.InstanceType == "" {
			...
			mpool.InstanceType, err = aws.PreferredInstanceType(ctx, installConfig.AWS, awsdefaults.InstanceTypes(installConfig.Config.Platform.AWS.Region, installConfig.Config.Arbiter.Architecture, topology), mpool.Zones)
			if err != nil {
				logrus.Warn(errors.Wrap(err, "failed to find default instance type"))
				mpool.InstanceType = awsdefaults.InstanceTypes(installConfig.Config.Platform.AWS.Region, installConfig.Config.Arbiter.Architecture, topology)[0]
			}
		}
		...
		for i, _ := range machines {
			if machines[i].Spec.ObjectMeta.Labels == nil {
				machines[i].Spec.ObjectMeta.Labels = map[string]string{}
			}
			machines[i].Spec.ObjectMeta.Labels["machineconfiguration.openshift.io/role"] = "arbiter"
			machines[i].Spec.Taints = []corev1.Taint{
				{
	...
	case baremetaltypes.Name:
		...
		arbiterUserDataSecretName = "arbiter-user-data-managed"
		...
		hostSettings, err := baremetal.Hosts(ic, machines, "arbiter", arbiterUserDataSecretName)
		if err != nil {
